...
const findByName = (name) => {
    return new Promise((resolve,reject) => {
        setTimeout(() => {
            const filterUsersName = users.find(user => user.name === name);
            resolve(filterUsersName);
        },2000);
    });
};
...
const filterByMajor = (major) => {
    return new Promise((resolve,reject) => {
        setTimeout(() => {
            const filterUsersMajor = users.filter(user => user.major === major);
            resolve(filterUsersMajor);
        },2000);
    });
};
